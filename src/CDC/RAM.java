...
public class RAM {
	...
	public RAM(String directory,String email) {
		indexTable = new Hashtable<String, String>();
		sum = new Checksum();
		// list = new FileList(Config.DIRECTORY);
		list = new FileList(directory+email);
	...
	public void displayChunks(File filelocation) {
		...
		try {
			...
			while (curr < length) {
				if ((hash & mask) == 0) {
					...
					if (fsChunk.read(chunk) != -1) {
						...
						hashvalue = sum.naechunking(chunk,length,window);
			...
			//docreg.setid(id);
			reg.setram_chks(count);
			if (duplicate != 0) {
			reg.setra_dup(duplicate);
			}
			else
			{
			reg.setra_dup(duplicate);
			}
			reg.setfilename(f.getName());
		    ...
		    int status=i.fileupdateRam(reg);
